...
for i_opt = 1:2,
    ...
    options = optimset('display','iter','diffmaxchange',1.1*1e-5, ...
        'diffminchange',1e-5,'MaxFunEvals',200000,'Maxiter',5000);
    ...
    if i_opt == 1
        ...
        tic
        X_opt = fmincon(@(X)obj_cost(X,N_grid),X0,[],[],[],[],lb,ub,@(X)cons_fun_doubleIntegrator(X,type),options);
        toc
    end
